...
CREATE OR REPLACE FUNCTION "public"."handle_new_game"() RETURNS "trigger"
...
WITH
  ...
  random_colors AS (
    SELECT
      row_number() over () as row_num, color
    FROM
      (
        SELECT
          'red' AS color
...
INSERT INTO public.tile
(game_id, position, word_id, team)
SELECT
  new.id,
...
ALTER TABLE "public"."tile" ALTER COLUMN "id" ADD GENERATED BY DEFAULT AS IDENTITY (
    SEQUENCE NAME "public"."tile_id_seq"
    START WITH 1
    INCREMENT BY 1
    NO MINVALUE
    NO MAXVALUE
...
CREATE OR REPLACE TRIGGER "on_game_created" AFTER INSERT ON "public"."game" FOR EACH ROW EXECUTE FUNCTION "public"."handle_new_game"();



CREATE OR REPLACE TRIGGER "on_room_created" AFTER INSERT ON "public"."room" FOR EACH ROW EXECUTE FUNCTION "public"."handle_new_room"();
...
ALTER TABLE ONLY "public"."player"
    ADD CONSTRAINT "player_current_room_id_fkey" FOREIGN KEY ("current_room_id") REFERENCES "public"."room"("id") ON DELETE SET NULL;



ALTER TABLE ONLY "public"."room"
    ADD CONSTRAINT "room_current_game_id_fkey" FOREIGN KEY ("current_game_id") REFERENCES "public"."game"("id") ON DELETE SET NULL;
...
CREATE POLICY "Enable insert for authenticated users only" ON "public"."player" FOR INSERT TO "authenticated" WITH CHECK ((( SELECT "auth"."uid"() AS "uid") = "id"));
...
CREATE POLICY "Enable update for authenticated users" ON "public"."game" FOR UPDATE TO "authenticated" USING (true);



CREATE POLICY "Enable update for authenticated users" ON "public"."tile" FOR UPDATE TO "authenticated" USING (true);



CREATE POLICY "Games are viewable by everyone" ON "public"."game" FOR SELECT USING (true);



CREATE POLICY "Public profiles are viewable by everyone." ON "public"."player" FOR SELECT USING (true);
...
CREATE POLICY "Tiles are viewable by everyone" ON "public"."tile" FOR SELECT USING (true);



CREATE POLICY "Users can update own profile." ON "public"."player" FOR UPDATE USING ((( SELECT "auth"."uid"() AS "uid") = "id"));



CREATE POLICY "Words are viewable by everyone" ON "public"."word" FOR SELECT USING (true);
...
ALTER PUBLICATION "supabase_realtime" ADD TABLE ONLY "public"."tile";
